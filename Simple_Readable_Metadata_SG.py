...
class SimpleReadableMetadataSG:
    ...
    RETURN_TYPES = ("STRING", "IMAGE", "MASK", "STRING", "STRING", "STRING", "INT")
    RETURN_NAMES = ("Simple_Readable_Metadata", "image", "mask", "metadata_raw", "Positive_Prompt", "Negative_Prompt", "seed")
    ...
    def load_analyze_extract(self, image, emoji_in_readable_text=True):
        ...
        return {
            "ui": {"text": lines},
            "result": (Simple_Readable_Metadata, image_tensor, mask, metadata_raw, positive, negative, seed_int)
        }
    ...
    def detect_format(self, text):
        ...
        if re.search(r'Steps:\s*\d+', text) or re.search(r'Sampler:\s*\w+', text) or re.search(r'CFG scale:\s*[\d.]+', text):
            return "webui"
        ...
        try:
            ...
            for node_id, node_data in data.items():
                if node_data.get("class_type") == "KSampler":
                    ksampler_data = node_data["inputs"]
                    sampling_params['seed'] = ksampler_data.get('seed', 'N/A')
                    sampling_params['steps'] = ksampler_data.get('steps', 'N/A')
                    sampling_params['cfg'] = ksampler_data.get('cfg', 'N/A')
                    sampling_params['sampler'] = ksampler_data.get('sampler_name', 'N/A')
                    sampling_params['scheduler'] = ksampler_data.get('scheduler', 'N/A')
                    sampling_params['denoise'] = ksampler_data.get('denoise', 'N/A')
                    break
            ...
            if not ksampler_data:
                for node_id, node_data in data.items():
                    ...
                    if "Noise" in class_type and "noise_seed" in inputs:
                        sampling_params['seed'] = inputs.get('noise_seed', 'N/A')
                    elif "Noise" in class_type and "seed" in inputs:
                        sampling_params['seed'] = inputs.get('seed', 'N/A')
                    
                    if "Scheduler" in class_type:
                        if 'steps' in inputs:
                            sampling_params['steps'] = inputs.get('steps', 'N/A')
                        if 'scheduler' in inputs:
                            sampling_params['scheduler'] = inputs.get('scheduler', 'N/A')
                        if 'denoise' in inputs:
                            sampling_params['denoise'] = inputs.get('denoise', 'N/A')
                    
                    if "CFG" in class_type and 'cfg' in inputs:
                        sampling_params['cfg'] = inputs.get('cfg', 'N/A')
                    
                    if "KSamplerSelect" in class_type and 'sampler_name' in inputs:
                        sampling_params['sampler'] = inputs.get('sampler_name', 'N/A')
            ...
            for node_id, node_data in data.items():
                ...
                if class_type == "CLIPTextEncodeFlux":
                    if isinstance(inputs, dict):
                        ...
                        if clip_l or t5xxl:
                            flux_prompts["clip_l"] = clip_l
                            flux_prompts["t5xxl"] = t5xxl
                    ...
                    if isinstance(text_value, dict):
                        if 'on' in text_value and not text_value.get('on'):
                            continue
                        text_value = text_value.get('text', text_value.get('value', text_value.get('prompt', '')))
                ...
                if "lora" in class_type.lower():
                    for key in inputs:
                        ...
                        if 'lora' in key_lower and inputs.get(key) not in [None, "", "None"]:
                            ...
                            if isinstance(lora_value, dict):
                                ...
                                if 'lora' in lora_value:
                                    ...
                                    if actual_lora_name and actual_lora_name != "None":
                                        display_name = os.path.basename(actual_lora_name)
                            ...
                            if lora_value and lora_value != "None" and isinstance(lora_value, str):
                                display_name = os.path.basename(lora_value)
                            ...
                            if param_name in inputs:
                                ...
                                if model_value and model_value != "None":
                                    ...
                                    if display_type not in models:
                                        models[display_type] = model_value
                                    
                                    if model_type != 'clip':
                                        break
                    
                    for key, value in inputs.items():
                        # Skip node references (lists)
                        ...
                        if isinstance(value, str) and any(ext in value.lower() for ext in ['.safetensors', '.ckpt', '.pt', '.pth', '.bin', '.gguf']):
                            key_lower = key.lower()
                            inferred_type = "Model"
    ...
    def extract_individual_params(self, text, format_type):
        ...
        try:
            if format_type == "comfyui":
                ...
                for node_id, node_data in data.items():
                    ...
                    if class_type == "CLIPTextEncodeFlux":
                        ...
                        positive = inputs.get("t5xxl", inputs.get("clip_l", ""))
            ...
            print(f"Error extracting individual params: {e}")
